...
import { Database, DealStatus } from '../types';
import { notifyChannelOwner, notifyUser } from './notifications';
import { schedulePost } from './posting';
import { fundDeal } from './wallet';
 ...
 */
export async function create_campaign(dealData: DealInsert) {
    ...
    if (data && deal.channel_id && deal.brief_text) {
        ...
        notifyChannelOwner(deal.channel_id, data.id, deal.brief_text).catch(console.error);
    }
 ...
 */
export async function approve_campaign(dealId: string, approverId: string, isAdvertiser: boolean = false, reject: boolean = false) {
    ...
    const updates: DealUpdate = { updated_at: new Date().toISOString() };
    ...
    if (reject) {
        ...
        await fundDeal(dealId, deal.advertiser_id, Number(deal.price_amount), deal.price_currency || 'USD');
    ...
    if (newStatus) {
        ...
        if (newStatus === 'funded' && updatedDeal) {
            ...
            schedulePost(updatedDeal.id, updatedDeal.channel_id || '', updatedDeal.creative_content).catch(console.error);

            notifyChannelOwner(updatedDeal.channel_id, updatedDeal.id, `🚀 Creative Approved!\n\nThe advertiser approved your post. It will be auto-posted at the scheduled time.`).catch(console.error);
        }

        return updatedDeal as DealRow;
    }
