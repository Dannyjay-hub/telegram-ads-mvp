 ...
 */
export async function verify_channel_admin(telegramChannelId: number) {
    if (!bot) {
        throw new Error('Bot not initialized');
    }

    try {
        const chatMember = await bot.api.getChatMember(telegramChannelId, bot.botInfo.id);

        if (chatMember.status === 'administrator' || chatMember.status === 'creator') {
            // Fetch real channel info
            const chatInfo = await bot.api.getChat(telegramChannelId);
            ...
            // To get member count:
            const memberCount = await bot.api.getChatMemberCount(telegramChannelId);
 ...
 */
export async function register_channel(channelData: ChannelInsert) {
    ...
    const providedStats = (channelData.verified_stats as any) || {};
