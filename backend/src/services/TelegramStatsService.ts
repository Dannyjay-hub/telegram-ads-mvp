...
export class TelegramStatsService {
    ...
    async getChannelStats(telegramChannelId: number) {
        if (!apiId || !apiHash) {
        ...
        if (!client) await this.initialize();
        ...
        try {
            ...
            const currentFollowers = (stats.followers as any)?.current || 0;
            const avgViews = (stats.viewsPerPost as any)?.current || 0;
