...
if (botInstance_1.bot) {
    ...
    botInstance_1.bot.command('negotiate', async (ctx) => {
        ...
        if (!telegramId || !dealId)
            return ctx.reply("Usage: /negotiate <deal_id>");
    ...
    botInstance_1.bot.command('accept', async (ctx) => {
        ...
        if (!dealId)
            return ctx.reply("You are not in a negotiation. Use /negotiate <deal_id> first.");
        const deal = await dealRepo.findById(dealId);
        if (!deal)
            return ctx.reply("Deal not found.");
        ...
        try {
            ...
            const member = await (0, telegram_1.getChatMember)(channel.telegramChannelId, telegramId);
