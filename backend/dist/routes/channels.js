...
app.post('/:id/verify-team', async (c) => {
    try {
        ...
        const { data: channel, error: channelError } = await db_1.supabase
            .from('channels')
        ...
        const { data: admins } = await db_1.supabase
            .from('channel_admins')
        ...
        const result = await (0, telegram_1.verifyTeamPermissions)(channel.telegram_channel_id, channel.owner_id, prManagers);
...
app.get('/:id/eligible-pr-managers', async (c) => {
    try {
        ...
        const { data: currentAdmins } = await db_1.supabase
            .from('channel_admins')
            .select('users(telegram_id)')
            .eq('channel_id', id);
        const existingTelegramIds = (currentAdmins || []).map((a) => a.users?.telegram_id);
        ...
        const eligible = telegramAdmins.filter((ta) => !existingTelegramIds.includes(ta.user.id) && !ta.user.is_bot);
        return c.json(eligible.map((e) => ({
...
app.post('/:id/pr-managers', async (c) => {
    try {
        ...
        const { data: ownerCheck, error: ownerError } = await db_1.supabase
            .from('channel_admins')
        ...
        console.log('[PR Manager] Ownership check:', {
            ...
            ownerCheck,
            ownerError,
            ownerTelegramId: ownerCheck?.users?.telegram_id
        });
        if (!ownerCheck || ownerCheck.users?.telegram_id !== requesterId) {
        ...
        // Get Telegram admins for this channel
        const telegramAdmins = await channelService.getTelegramAdmins(channel.telegramChannelId);
        console.log('[PR Manager] Telegram admins:', telegramAdmins.map((a) => ({ id: a.user.id, username: a.user.username })));
        // If username provided, find the admin by username
        let targetAdmin = null;
        if (username) {
            username = username.replace('@', '').toLowerCase();
            targetAdmin = telegramAdmins.find((ta) => ta.user.username?.toLowerCase() === username);
            if (!targetAdmin) {
            ...
            targetAdmin = telegramAdmins.find((ta) => ta.user.id === telegram_id);
            if (!targetAdmin) {
        ...
        // Create or get user
        let { data: existingUser } = await db_1.supabase
            .from('users')
            .select('id')
            .eq('telegram_id', telegram_id)
            .single();
        if (!existingUser) {
            const telegramUser = telegramAdmins.find((ta) => ta.user.id === telegram_id)?.user;
            const { data: newUser } = await db_1.supabase
                .from('users')
                .insert({
                telegram_id,
        ...
        const { data: existingAdmin } = await db_1.supabase
            .from('channel_admins')
            .select('role')
            .eq('channel_id', id)
...
app.delete('/:id/pr-managers/:telegramId', async (c) => {
    try {
        ...
        const { data: pmUser, error: userError } = await db_1.supabase
            .from('users')
            .select('id')
        ...
        const { error: deleteError, count } = await db_1.supabase
            .from('channel_admins')
            .delete()
            .eq('channel_id', id)
...
app.post('/verify_permissions', async (c) => {
    try {
        ...
        const resolvedId = await (0, telegram_1.resolveChannelId)(channel_id);
        if (!resolvedId) {
            return c.json({
                status: 'error',
        ...
        if (!skip_existing_check) {
            ...
            const { data: existingChannel, error: checkError } = await db_1.supabase
                .from('channels')
        ...
        const userMember = await (0, telegram_1.getChatMember)(resolvedId, userId);
        ...
        // 4. State D: Ready
        const channelStats = await (0, telegram_1.getChannelStats)(resolvedId);
        if (!channelStats) {
        ...
        return c.json({
            ...
            channel_details: {
                ...
                avg_views: channelStats.avg_views
            },
...
app.post('/', async (c) => {
    try {
        const body = await c.req.json();
        const { telegram_channel_id, status, description, category, tags, base_price_amount, pricing, rateCard } = body;
